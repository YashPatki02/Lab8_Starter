describe('Basic user flow for Website', () => {
  ...
  it('Make sure <product-item> elements are populated', async () => {
    ...
    for (let idx = 0; idx < prodItems.length; idx++) {
      console.log(`Checking product item ${idx+1}/${prodItems.length}`);
      // Grab the .data property of <product-items> to grab all of the json data stored inside
      data = await prodItems[idx].getProperty('data');
  ...
  it('Checking number of items in cart on screen', async () => {
    ...
    for (let idx = 1; idx < prodItems.length; idx++) {
    // get the shadowRoot and query select the button inside, and click on it.
      let shadowRoot = await prodItems[idx].getProperty('shadowRoot');
      const button = await shadowRoot.$('button');
      await button.click();
    }
